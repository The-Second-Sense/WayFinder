...
const CardsScreen = () => {
  ...
  const newCardFormat: "VIRTUAL" = "VIRTUAL";
  const [pinModalVisible, setPinModalVisible] = useState(false);
  const [pinError, setPinError] = useState<string | null>(null);
  const [pinLoading, setPinLoading] = useState(false);
  const [pendingCardAction, setPendingCardAction] = useState<{ type: "toggle" | "delete"; cardId: string } | null>(null);
  ...
  const requestPinForAction = (action: { type: "toggle" | "delete"; cardId: string }) => {
    setPendingCardAction(action);
    setPinError(null);
    setPinModalVisible(true);
  };

  const executeCardAction = (action: { type: "toggle" | "delete"; cardId: string }) => {
    ...
    setCards((prev) => prev.filter((card) => card.id !== action.cardId));
  };

  const handleConfirmPinForCardAction = async (pin: string) => {
    ...
    try {
      ...
      executeCardAction(pendingCardAction);
      setPinModalVisible(false);
      setPendingCardAction(null);
    } catch (error: any) {
      setPinError(error?.message || "Eroare la verificarea PIN-ului.");
    } finally {
      setPinLoading(false);
    }
  };
  ...
  const resolveDisplayedCardNumber = (card: MockCard) => card.maskedPan;
      ...
      >
        ...
        <View style={styles.cardActions}>
          <TouchableOpacity
            ...
            onPress={() => requestPinForAction({ type: "toggle", cardId: item.id })}
          >
            <Power size={16} color={textColor} />
            <Text style={[styles.cardActionText, { color: textColor }]}>
              {item.isActive ? "Blochează card" : "Deblochează card"}
            </Text>
          </TouchableOpacity>
          ...
          <TouchableOpacity
            style={styles.cardActionButton}
            onPress={() =>
              Alert.alert("Ștergere card", "Confirmi ștergerea cardului?", [
                { text: "Anulează", style: "cancel" },
                {
                  text: "Șterge",
                  style: "destructive",
                  onPress: () => requestPinForAction({ type: "delete", cardId: item.id }),
                },
              ])
            }
          >
    ...
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.toolbar}>
        <Text style={styles.toolbarText}>
          Cardurile sunt locale și pot fi gestionate direct din aplicație.
        </Text>
          ...
          <View style={styles.modalContainer}>
            ...
            <View style={styles.optionRow}>
              <View style={[styles.optionPill, styles.optionPillActive]}>
                <Text style={[styles.optionPillText, styles.optionPillTextActive]}>VIRTUAL</Text>
              </View>
            </View>
      ...
      <PinConfirmationModal
        ...
        title="Confirmare PIN"
        message="Introdu PIN-ul de transfer pentru această acțiune"
        error={pinError}
        loading={pinLoading}
        onCancel={() => {
          setPinModalVisible(false);
        ...
        onConfirm={handleConfirmPinForCardAction}
      />
    </SafeAreaView>
  );
};
